...
function Users() {
    ...
    const [, setFormValues] = useState<DataType>({} as DataType);
   ...
   const disabled = selectedRowKeys.length === 0
    ...
    const loadData = async () => {
        setLoading(true)
        try {
            const res = await getUserList({ ...formData, page, pageSize }) as UserListResponse;
            setDataList(res.data.list as DataType[])
    ...
    const batchDelete=async ()=>{
        const res = await batchDeleteUser(selectedRowKeys) as any;
        message.success(res?.data || '删除成功');
        loadData();
    }
    ...
    const handleFormValuesChange = useCallback((_values: any, allValues: DataType) => {
