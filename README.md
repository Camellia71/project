...
#### 为什么做？
   “在 SmartPark_ZQY 这个智慧园区项目中，随着仪表盘、设备管理、用户管理等模块越来越多，我们遇到了几个比较典型的大型系统加载体验问题。
   第一个是首屏白屏时间长，首次加载 JS 包体积大，用户要等 2 到 3 秒才能看到内容，期间屏幕完全是空白的.第二个是加载状态太单一，我们一开始只在入口放了一个简单的 Spin 组件，用户不知道页面什么时候能出来，对即将看到的内容也没有任何预期。第三个就是骨架屏代码重复严重，最初我们是每个页面单独写一套骨架屏，样式不统一，改一个占位符颜色或者动画效果就得同步改好几个文件。
   最关键的是，引入 React.lazy 做路由懒加载之后，虽然单个组件的加载变快了，但路由切换时出现了明显的空白期，反而让用户感知更差了。
##### 为什么要「原子化」？
- 如果直接写一整个页面的骨架屏，代码太臃肿，难以复用
- 拆成小积木后， SkeletonAvatar 可以在用户列表、个人中心等多个地方复用
- SkeletonParagraph 可以调整行数和宽度，适配不同场景
##### 为什么要「预设布局」？
- 项目中大部分页面可以归为表单、列表、详情、仪表盘四种类型
- 预设布局能让开发人员快速接入，不用每次都从零开始拼积木
- 保持整个系统的骨架屏风格统一
#### 简单讲一下怎么做的？
   为此我们构建了一套原子化骨架屏体系，核心思路是把骨架屏拆成小积木再拼出常用模板——先抽象出 SkeletonAvatar 头像占位和 SkeletonParagraph 文字占位这两个最小原子单元，然后基于它们封装了表单、列表、详情、仪表盘四种预设布局的 PageSkeleton。使用时在路由配置中配合 React.lazy 和 Suspense，系统会根据页面路径自动匹配对应骨架屏，比如访问 /users 展示列表骨架、访问 /dashboard 展示仪表盘骨架，页面加载完成后自动替换为真实内容。
   这样做既让 SkeletonAvatar 等原子组件可以在用户列表、个人中心等多个场景复用，避免了代码重复，又通过预设布局保持了整个系统骨架屏风格统一，开发人员也能快速接入，最终彻底解决了首屏白屏问题，让用户在任何页面都能立刻看到一个与最终内容结构一致的占位界面。
#### 实现
...
#### 为什么做？
   在 SmartPark_ZQY 项目中，我们把 LazyImage 和 VirtualList 结合起来使用，是因为它们解决的是数据密集型页面两个不同维度的性能问题——VirtualList 解决的是 DOM 渲染层面的问题，只渲染可视区域的列表项，避免创建大量 DOM 节点；LazyImage 解决的是图片加载层面的问题，确保只有进入可视区域的图片才会发起网络请求。单独用其中一个都有短板：只用 VirtualList 的话，可视区域内的图片还是会一次性全部加载，比如设备管理页面可视区域有 20 条数据，这 20 条里的设备图片会立即全部请求；只用 LazyImage 的话，虽然图片懒加载了，但列表项本身还是全量渲染，DOM 节点依然很多。两者结合之后，VirtualList 控制渲染的列表项数量，LazyImage 在这些已渲染的列表项中进一步控制图片加载时机，首屏加载更快、滚动更流畅、内存和带宽占用也更低，实现了真正的双重性能优化。
##### 为什么要封装 LazyImage？
   在 SmartPark_ZQY 项目中，我们遇到了一个典型问题：设备管理页面和房间展示页面包含大量图片，首次进入时浏览器会同时发起几十甚至上百个图片请求。这不仅导致首屏加载时间大幅增加，还会阻塞其他关键资源的加载。比如设备列表页面，每个设备卡片都有一张缩略图，如果一次性加载所有设备图片，网络请求队列会被占满，用户需要等待很长时间才能看到页面内容。
   传统的图片加载方式无法感知图片是否在可视区域内，只能按顺序加载。我们尝试过使用滚动监听来判断，但频繁的 scroll 事件会触发大量重排，反而影响性能。IntersectionObserver API 完美解决了这个问题——它能异步监听元素进入视口的状态，而且不会频繁触发回调。基于这个 API 封装的 LazyImage 组件，只有当图片进入可视区域时才会发起加载请求，有效减少了首屏请求数量，让关键内容能更快地呈现给用户。
##### 为什么要封装 VirtualList？
   智慧园区管理系统需要处理大量数据列表，比如设备管理、用户管理、合同管理等模块，每个模块的数据量都可能达到上千条。最初我们使用普通的 Table 组件直接渲染所有数据，很快就遇到了性能瓶颈：当列表数据超过 500 条时，页面渲染时间明显增加，滚动时会出现明显的卡顿甚至掉帧。
   这是因为浏览器需要为每个列表项创建 DOM 节点，大量的 DOM 操作会触发频繁的重排和重绘。我们计算过，如果渲染 1000 条设备数据，会生成数千个 DOM 元素，这对浏览器来说是巨大的负担。VirtualList 的出现就是为了解决这个问题——它通过 IntersectionObserver 或滚动监听，只渲染可视区域内的列表项，其他数据则用空白占位。比如屏幕能显示 20 条数据，就只渲染这 20 条，再加上前后各 3 条的缓冲，总共只有 26 个 DOM 节点，性能提升非常明显。
#### 简单讲一下怎么做的？
   图片懒加载这块，我核心用的是 IntersectionObserver。组件有四个状态：idle、loading、loaded、error。挂载后我用 Observer 盯着图片容器节点，设置 rootMargin 提前 100 像素触发，一旦容器进入可视区，状态切到 loading，并把真实图片地址存起来。然后另一个 effect 监听到地址变化，用 new Image 在内存里去加载这张图，成功就切 loaded 渲染 img 标签，失败就切 error。idle 和 loading 阶段展示占位符，error 时如果没有自定义错误占位就显示加载失败的文字。为了健壮性，我在图片加载的 effect 里加了取消标志，防止组件卸载后还去 setState，回调函数也用 ref 存了一份，避免它们变化导致 effect 重复触发。
   虚拟列表这块，核心思想是只渲染看得见的那部分元素。我维护了 scrollTop 和 clientHeight 两个状态。滚动的时候用 scrollTop 除以 itemHeight 算出当前滚到了第几个，用 clientHeight 除以 itemHeight 算出屏幕上能装几个，然后前后各加 overscan 个缓冲区，得到 startIndex 和 endIndex。渲染的时候用三层结构：最外层是固定高度的滚动容器，中间层高度设成总数据量乘单项高度来撑出滚动条，最内层用 translateY 把可见元素推到这个滚动位置的正确偏移上。另外用了 ResizeObserver 监听容器尺寸变化，窗口缩放时自动重算。这样无论列表有多少数据，实际渲染的 DOM 节点始终只有可视区加缓冲区那十几个二十几个。这两个组件配合使用的话，就是典型的图片列表场景，虚拟列表减少 DOM 数量，懒加载减少网络请求，双重的性能优化。
#### 实现
```
src/components/
├── LazyImage/
...
##### LazyImage 的实现原理
```js
// 核心逻辑：使用 IntersectionObserver 监听图片容器
const observer = new IntersectionObserver(
(entries) => {
    entries.forEach((entry) => {
    if (entry.isIntersecting) {
        setStatus('loading');
        setImageSrc(src);  // 进入视口才开始加载
...
```
工作流程 ：
1. 页面加载时，只渲染一个空的容器和占位符
2. IntersectionObserver 监听容器是否进入视口
3. 当容器进入视口（或距离视口 100px 内），才设置真实的图片地址
4. 通过 new Image() 预加载图片，加载完成后显示真实图片
##### VirtualList 的实现原理
```js
// 核心计算：确定需要渲染的列表项范围
...
```
工作流程 ：
1. 根据滚动位置 scrollTop 和容器高度 clientHeight ，计算可视区域的起始和结束索引
2. 只渲染 startIndex 到 endIndex 之间的列表项（默认前后各预渲染 3 项）
3. 通过 offsetY 设置偏移量，模拟完整列表的滚动效果
4. 滚动时更新 scrollTop ，重新计算渲染范围
##### 结合使用示例
```js
// 在设备管理页面中的使用示例
<VirtualList
...
```
协同工作流程 ：
1. VirtualList 只渲染可视区域的 20 条设备数据（假设屏幕能显示 20 条）
2. 这 20 条数据中的 LazyImage 组件开始监听自己是否进入视口
3. 当某条设备数据滚动到屏幕中央时，对应的 LazyImage 触发加载
4. 图片加载完成后，从占位符变为真实图片
##### 性能优化效果
![](assets/17781501485770.jpg)

### 6.动态路由，闭包+HOC权限控制
#### 简述
   设计路由初始化守卫配合 PrivateRouter 实现登录态拦截与越权阻断，基于 Token 动态注册路由消除刷新白屏与 404 误跳转；基于闭包 + HOC 封装按钮鉴权组件，权限逻辑内聚复用，消除业务侧散落 if 判断
#### 为什么做？
